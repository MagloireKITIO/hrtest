...
from horilla.horilla_middlewares import get_current_request
...
def get_pagination():
    """
    Récupère la configuration de pagination de manière sécurisée
    """
    # Valeur par défaut configurable
    DEFAULT_PAGINATION = getattr(settings, 'HORILLA_DEFAULT_PAGINATION', 50)
    ...
    try:
        ...
        if hasattr(request, '_pagination_cache'):
            return request._pagination_cache
            
        # Récupération de la configuration utilisateur
        page = DynamicPagination.objects.filter(user_id=request.user).select_related('user').first()
        
        # Stockage du résultat en cache pour la requête
        pagination_value = page.pagination if page else DEFAULT_PAGINATION
        request._pagination_cache = pagination_value
        
        return pagination_value
        
    except Exception as e:
        # Option: Log l'erreur si nécessaire
        # import logging
        # logger = logging.getLogger(__name__)
        # logger.error(f"Error in get_pagination: {str(e)}")
        return DEFAULT_PAGINATION
