...
class ChangePasswordForm(forms.Form):
    ...
    def __init__(self, user, *args, **kwargs):
        self.user = user
        unblocked = kwargs.pop('unblocked', False)  # Récupérer le flag unblocked
        super().__init__(*args, **kwargs)
        
        # Si l'utilisateur est débloqué, ne pas afficher old_password
        if unblocked:
            del self.fields['old_password']

    def clean_old_password(self):
        # Ne valider que si le champ existe (utilisateur non débloqué)
    ...
    def clean_new_password(self):
        ...
        # Validation complète du mot de passe avec tous les validateurs configurés
        try:
            # Utilise les validateurs de settings.py incluant ComplexPasswordValidator
        ...
        # Seulement si l'utilisateur n'est pas débloqué
        if not self.instance_state.get('unblocked', False):
            if self.user.check_password(new_password):
                raise forms.ValidationError(_("Le nouveau mot de passe doit être différent de l'ancien."))
    ...
    def instance_state(self):
        """Helper pour stocker l'état de l'instance"""
...
class MailTemplateForm(ModelForm):
    ...
    def get_template_language(self):
        mail_data = {
            ...
            "Receiver|Employee pwd":"instance.temp_password",
