...
class JobRole(HorillaModel):
    ...
    job_role = models.CharField(max_length=100, blank=False, null=True)
    company_id = models.ManyToManyField(Company, blank=True, verbose_name=_("Company"))
...
class LoginAuditLog(models.Model):
    """
    Model to track login attempts and user profile information
    """
    ...
    def get_user_roles(self):
        """Get user's groups/roles as comma separated string"""
        if not self.employee_id or not self.employee_id.employee_user_id:
            return "-"
        groups = self.employee_id.employee_user_id.groups.all()
        return ", ".join([g.name for g in groups]) if groups else "-"
    
    def get_last_password_change(self):
        """Get user's last password change date"""
        if not self.employee_id or not self.employee_id.employee_user_id:
            return None
        last_change = UserPasswordHistory.objects.filter(
            user=self.employee_id.employee_user_id
        ).first()
...
class UserPasswordHistory(models.Model):
    """
    Model to track user password changes
    """
    user = models.ForeignKey(User, on_delete=models.CASCADE)
