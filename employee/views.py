...
from django.forms import DateInput, Select, ValidationError
...
def employee_view_individual(request, obj_id, **kwargs):
    ...
    companies = employee.employee_work_info.company_id.all()
    employee_leaves = (
...
def employee_view(request):
    ...
    # Appliquer les restrictions de compagnie
    if not employee.employee_user_id.is_superuser and employee.company_id:
        # Filtrer pour ne montrer que les employés de la même compagnie
        queryset = queryset.filter(company_id=employee.company_id)
        # Si l'employé est un manager, ajouter ses subordonnés
        if employee.employee_work_info.reporting_manager_id:
            subordinates = queryset.filter(
                employee_work_info__reporting_manager_id=employee
            )
            queryset = (queryset | subordinates).distinct()
    ...
    # Filtrer emp également par compagnie
    if not employee.employee_user_id.is_superuser and employee.company_id:
        emp = Employee.objects.filter(company_id=employee.company_id)
    else:
    ...
    context = {
        ...
        "user_company": employee.get_company_name(),  # Ajout du nom de la compagnie
        "is_superuser": employee.employee_user_id.is_superuser,
    }
...
def employee_create_update_personal_info(request, obj_id=None):
    ...
    try:
        if form.is_valid():
            if obj_id is None:  # Création
                ...
                if User.objects.filter(username=form.cleaned_data['email']).exists():
                    ...
                    return HttpResponse(
                        f'<ul class="alert alert-danger"><li>{_("Un utilisateur avec cet email existe déjà.")}</li></ul>'
                    )
                ...
                employee.save()
                messages.success(request, _("New Employee Added Successfully"))
                return HttpResponse("<script>window.location.href = '/employee/employee-view/?view=list';</script>")
                ...
                if form.cleaned_data.get('is_superuser'):
                    ...
                    if not form.cleaned_data.get('company_id'):
                        raise ValidationError(_("Company is required for non-superuser employees"))
                    employee.company_id = form.cleaned_data['company_id']
                    employee.employee_user_id.is_superuser = False
                    employee.employee_user_id.is_staff = False
                
                employee.employee_user_id.save()
                employee.save()

                messages.success(request, _("Employee Information Updated Successfully"))
                return HttpResponse("<script>window.location.href = '/employee/employee-view/?view=list';</script>")
        ...
        if employee.employee_user_id:
            employee.employee_user_id.delete() 
        if employee.pk:
            employee.delete()
...
def employee_update_work_info(request, obj_id=None):
    """
    Update employee work info
    """
    employee = Employee.objects.get(id=obj_id)
    instance = EmployeeWorkInformation.objects.get(employee_id=employee)
    
    # Utiliser le formulaire de mise à jour spécifique
    ...
    if form.is_valid():
        # Sauvegarder sans commit d'abord
        work_info = form.save(commit=False)
        work_info.employee_id = employee
        work_info.save()
        
        # Gérer les relations many-to-many après la sauvegarde
        if 'company_id' in request.POST:
            work_info.company_id.set(request.POST.getlist('company_id'))
    ...
    # En cas d'erreur, retourner les erreurs
    errors = "\n".join([f"<li>{field}: {error}</li>" for field, error in form.errors.items()])
    return HttpResponse(f'<ul class="alert alert-danger">{errors}</ul>')
