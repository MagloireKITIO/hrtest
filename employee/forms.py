...
class EmployeeForm(ModelForm):
    ...
    company_id = forms.ModelChoiceField(
        ...
        empty_label=_("All Companies (Super User)"),
        required=False,
        label=_("Company"),
        widget=forms.Select(
            attrs={
    ...
    phone = forms.CharField(
        max_length=15,
        label=_("Phone"), # Ajout du label explicite
    ...
    def clean(self):
        ...
        # Permettre le changement de compagnie seulement si :
        # 1. L'utilisateur qui modifie est un superuser OU
        # 2. L'utilisateur qui modifie a accès à toutes les compagnies
        request = getattr(horilla_middlewares._thread_locals, "request", None)
        
        if (self.instance.pk and  # Modification d'un employé existant
            self.instance.company_id and  # L'employé avait déjà une compagnie
            not self.instance.employee_user_id.is_superuser and  # L'employé n'est pas superuser
            request and request.user.employee_get.company_id):  # L'utilisateur qui modifie n'est pas superuser
            
            # Vérifier si l'utilisateur essaie de changer la compagnie
            new_company = cleaned_data.get('company_id')
            if new_company != self.instance.company_id:
                # Permettre seulement si l'utilisateur qui modifie est superuser
                if not request.user.is_superuser:
                    raise forms.ValidationError({
                        'company_id': _("Seuls les super-utilisateurs peuvent changer la compagnie d'un employé.")
                    })
    ...
    def save(self, commit=True):
        ...
        if commit:
            ...
            if self.cleaned_data.get('group'):
                instance.employee_user_id.groups.add(self.cleaned_data['group'])
...
class EmployeeWorkInformationForm(ModelForm):
    ...
    # Ajout du champ companies
    companies = forms.ModelMultipleChoiceField(
        queryset=Company.objects.all(),
        widget=forms.SelectMultiple(attrs={
            'class': 'oh-select oh-select-2 select2-hidden-accessible',
            'data-placeholder': _('Select Companies'),
        }),
        required=False,
        label=_("Companies")
    )
    ...
    def __init__(self, *args, disable=False, **kwargs):
        ...
        # Initialisation des companies si instance existe
        if self.instance and self.instance.pk:
            self.fields['companies'].initial = self.instance.company_id.all()
    ...
    def save(self, commit=True):
        ...
        if commit:
            ...
            if self.cleaned_data.get('companies'):
                instance.company_id.set(self.cleaned_data['companies'])
        return instance
...
class EmployeeWorkInformationUpdateForm(ModelForm):
    ...
    Form for EmployeeWorkInformation model
    """
    # Ajout du champ companies pour le formulaire de mise à jour
    companies = forms.ModelMultipleChoiceField(
        queryset=Company.objects.all(),
        widget=forms.SelectMultiple(attrs={
            'class': 'oh-select oh-select-2 select2-hidden-accessible',
