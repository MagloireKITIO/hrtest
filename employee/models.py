...
class Employee(models.Model):
    ...
    company_id = models.ForeignKey(
        ...
        help_text=_("Employee will only see data from this company. Leave blank for all companies access.")
    )
    ...
    is_selector = models.BooleanField(
        default=False, 
        verbose_name=_("Is Selector"),
        help_text=_("If selected, this employee will only see assigned recruitments")
    )
    ...
    def increment_failed_attempts(self):
        ...
        if self.failed_login_attempts >= 10:  # Max attempts
            self.employee_user_id.is_active = False
        self.save()
    ...
    def get_visible_recruitments(self):
        """
        Méthode helper pour obtenir les recrutements visibles avec vérification de sécurité
        """
        from recruitment.models import Recruitment
        
        if self.is_selector:
            # Récupérer les recrutements assignés au sélecteur
            base_query = Recruitment.objects.filter(selectors=self)
            
            # CORRECTION SÉCURITÉ : Vérifier la compagnie même pour les sélecteurs
            if not self.employee_user_id.is_superuser and self.company_id:
                base_query = base_query.filter(company_id=self.company_id)
            
            return base_query
            
        elif not self.employee_user_id.is_superuser and self.company_id:
            # Employé normal : voit les recrutements de sa compagnie
            return Recruitment.objects.filter(company_id=self.company_id)
        else:
            # Superuser : voit tous les recrutements
            return Recruitment.objects.all()

    def get_company_name(self):
        """Returns company name or 'All Companies' for superusers"""
        if self.employee_user_id and self.employee_user_id.is_superuser:
            return _("All Companies")
        return str(self.company_id.company) if self.company_id else _("No Company")

    def get_selector_display(self):
        """Returns Yes/No for is_selector"""
        return _("Yes") if self.is_selector else _("No")

    def has_all_company_access(self):
        """
        Returns True if the employee has access to all companies
        """
    ...
    class Meta:
        ...
        permissions = (
            ...
            ("can_unblock_users", "Can unblock locked user accounts"),
        )
        ordering = [
            "employee_first_name",
        ]
