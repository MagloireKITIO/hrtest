...
from candidate_portal.models import CandidateAuth, CandidateProfile
...
class RecruitmentFilter(FilterSet):
    ...
    selectors = django_filters.ModelChoiceFilter(
        queryset=Employee.objects.filter(is_selector=True),
        label=_("Demandeur"),
        widget=forms.Select(
            attrs={
                "class": "oh-select oh-select-2",
                "placeholder": _("Select Selector")
            }
        )
    )
...
class CandidateAuthFilter(FilterSet):
    """
    Filtre pour les comptes candidats (CandidateAuth)
    """
    email = django_filters.CharFilter(field_name="email", lookup_expr="icontains")
    name = django_filters.CharFilter(method="filter_by_name")
    company = django_filters.ModelChoiceFilter(
        queryset=Company.objects.all(),
        field_name="profile__company"
    )
    ...
    # Filtre par statut de connexion (jamais connecté/connecté)
    login_status = django_filters.ChoiceFilter(
        choices=[('connected', 'Connecté'), ('never', 'Jamais connecté')],
        method="filter_by_login_status"
    )
    ...
    # Filtre par niveau de complétion du profil
    ...
    def filter_by_login_status(self, queryset, name, value):
        """
        Filtre par statut de connexion
        """
        if value == 'connected':
            return queryset.exclude(last_login=None)
        elif value == 'never':
            return queryset.filter(last_login=None)
        return queryset

    def filter_by_completion(self, queryset, name, value):
        """
        Filtre par niveau de complétion du profil
        """
        # Récupérer tous les IDs de candidats
        candidate_ids = queryset.values_list('id', flat=True)
        
        # Récupérer les profils correspondants
        profiles = CandidateProfile.objects.filter(candidate_auth_id__in=candidate_ids)
        
        # Filtrer selon le niveau de complétion
        if value == 'low':
            # Profils avec moins de 25% de complétion
            filtered_ids = [
                p.candidate_auth_id for p in profiles 
                if hasattr(p, 'get_profile_completion_percentage') and p.get_profile_completion_percentage() < 25
            ]
        elif value == 'medium':
            # Profils entre 25% et 75% de complétion
            filtered_ids = [
                p.candidate_auth_id for p in profiles 
                if hasattr(p, 'get_profile_completion_percentage') 
                and 25 <= p.get_profile_completion_percentage() < 75
            ]
        elif value == 'high':
            # Profils avec plus de 75% de complétion
            filtered_ids = [
                p.candidate_auth_id for p in profiles 
                if hasattr(p, 'get_profile_completion_percentage') and p.get_profile_completion_percentage() >= 75
            ]
        else:
            return queryset
        
        # Retourner les candidats filtrés
