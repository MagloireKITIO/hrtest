...
def selected_format(date: datetime.date, company: object = None) -> str:
    """
    Format date/time based on company preferences.
    Handles cases where company might be a ManyRelatedManager or single instance.
    
    Args:
        date: datetime.date or datetime.time object to format
        company: Company instance or ManyRelatedManager object
        
    Returns:
        str: Formatted date/time string
    """
    # Return original date if no company provided
    if not company:
        return date

    # Handle case where company is a ManyRelatedManager
    if hasattr(company, 'all'):
        try:
            # Get the first company instance
            company = company.first()
        except Exception:
            return date

    # Check if we have a valid company instance with format preferences
    if company and hasattr(company, 'date_format') and hasattr(company, 'time_format'):
        if isinstance(date, datetime.date):
            format = company.date_format
            if format in date_format_mapping:
                return date.strftime(date_format_mapping[format])
        elif isinstance(date, datetime.time):
            format = company.time_format
            if format in time_format_mapping:
...
def format(string: str, instance: object):
    """
    Format a string using attributes from an instance
    """
...
def accessibility(method: str, instance=None):
    """
    Check accessibility permissions for a given method and instance
    """
    if method:
...
def col(field: object):
    """
    Method to get field col separation
    """
